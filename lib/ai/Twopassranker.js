 ...
 *
 * Old approach: keyword matching rubric → everyone gets 70-80
 * New approach: think like a real Staff-level recruiter at Google/Meta
 *
 * A real FAANG recruiter does NOT:
 * - Match keywords mechanically
 * - Give everyone a polite middle score
 * - Treat all experience as equal
 *
 * A real FAANG recruiter DOES:
 * - Read between the lines
 * - Assess trajectory and growth arc
 * - Judge impact by scale and ownership, not just tech listed
 * - Spot red flags that candidates try to hide
 * - Spot hidden gems that weak resumes accidentally reveal
 * - Compare candidates relatively, not against an abstract standard
 */
...
async function callGroq(system, user) {
  ...
  try {
    ...
    if (!res.ok) {
      const e = new Error(`Groq ${res.status}`);
      e.status = res.status;
      throw e;
    }
...
async function callGroqRetry(sys, usr, attempt = 1) {
  try {
...
// This is the core intelligence — NOT a rubric checker
...
function pass1UserPrompt({ candidateId, jd, resume }) {
...
async function scoreOne(candidate, jd) {
  try {
...
async function runPass1(candidates, jd, onProgress) {
  const results = [];
...
// Committee debrief — now compare them against each other
...
function pass2UserPrompt(candidates, jd) {
...
async function runPass2(pass1Results, jd, onProgress) {
  ...
  const GROUP = 10; // smaller groups = less tokens per call = less timeout risk
  const groups = [];
  ...
  for (let gi = 0; gi < groups.length; gi++) {
    ...
    try {
      ...
      console.error(`Pass2 group ${gi} failed: ${e.message} — falling back to pass1 scores`);
      // Always fallback gracefully — never hang
      group.forEach(c => {
        allRanked.push({
          candidate_id: c.id,
...
async function rankAllCandidates(candidates, jd, onProgress) {
  const pass1 = await runPass1(candidates, jd, onProgress);
  return runPass2(pass1, jd, onProgress);
}

module.exports = { rankAllCandidates };